...
impl ConfigReader {
    ...
    pub fn resolve_config_hierarchy(
        ...
        if let Some(env_path) = config_env.map(|s| PathBuf::from(s.trim())).filter(|p| p.exists()) {
            paths.push(env_path);
            return paths; 
        }
        ...
        if let Some(current) = current_dir {
            // 3. Конфиг организации (Родительская папка)
...
mod tests {
    ...
    fn test_hierarchy_loading_order() {
        ...
        // Создаем все папки и пустые файлы конфигов
